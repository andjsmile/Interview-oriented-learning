...
=default  我们需要默认构造函数，也需要其他形式的构造函数
...
## 第十四章 重载运算符与类型转换 (08.08)


递增和递减运算符 有前置和后置两个版本

前置运算符返回递增或递减后对象的引用  operator++()

后置运算符返回对象的原值，返回一个值  operator++(int)


箭头运算符 -> 重载函数 将其实际工作委托给 解引用运算符 * 
...
- 继承：定义与其他类相似，但不完全相同的新类

- 动态绑定:在一定程序上，忽略类的差别，可以使用统一的方式使用它们的对象

当我们使用基类的引用(指针)调用一个虚函数时，将发生动态绑定
...
某个类作为基类，该类必须已经定义，而不仅仅是声明

final ，类名后面加上final关键字，防止继承发生
...
基类的对象可能是派生类的一部分，也可能不是。


使用override在派生类中，可以检查出 派生类函数没有覆盖基类版本的错误
(派生类定义一个函数与基类中虚函数名字相同但是形参列表不同)
...
派生类访问说明符对于派生类的成员能否直接访问其直接基类的成员无影响

派生类对一个基类对象中的受保护成员没有任何访问权限

**某个类对其继承而来的成员访问权限受两个因素影响**
...
派生类可以访问 基类中public，protected 成员，private成员对于派生类是不可访问的

派生访问说明符是控制派生类用户(包括派生类的派生类)对基类成员的访问权限

如果继承是公有的public，则成员遵守原有的访问说明符


upper_bound() 是一个很有用的内置函数

可以令我们跳过当前关键字相同的所有元素

**友元不能传递，也不能继承**

派生类的作用域在基类的作用域内，嵌套在里面

**虚析构函数**

基类中将析构函数定义成虚函数，确保执行正确的析构函数

删除的是一个指向派生类对象的基类指针，则需要虚析构函数


派生类析构函数，只负责销毁由派生类自己分配的资源

派生类拷贝或移动构造函数，该操作负责拷贝或移动包括基类部分成员在内的整个对象
...
面对对象编程oop 泛型编程都能处理在编写程序时不知道类型情况
...
- 非类型参数  template<int N>,必须是常量表达式(指定数组的大小)
...
控制实例话，extern 表示声明  可以多次声明，但只能一次定义
...
定义一个随机种子,来实现运行产生不同的结果

normal_distribution
berboulli_distribution
uniform_int_distribution    均分布
...
get(sink,size ,delim)
getline(sink,size,delim)   delim  遇到截止符
...
捕获异常     catch(...) ，用在所有catch语句之后

nonexpect 说明    是一个不抛出异常说明
...
- runtimer_error  ,运行时错误，只有在程序运行的时候才能监测到的错误
  
   - overflow_error
   - underflow_error
   - range_error

- logic_error   ,逻辑错误，一般指的是我们可以在程序的代码中发现的错误
...
很多独立开发库，库又又会定义大量的全局名字，类、函数、模版

多个库将名字放置在全局命名空间中将引发命名空间污染 (namespace pollution)


利用命名空间防止名字冲突提供了更加可控的机制

命名空间分割了全局命名空间，其中每个命名空间都是一个作用域 

命名空间可以是不连续的，每一个命名空间都是一个作用域，与块类似不需要在分号后面加上";"


c++11 新标准，内联命名空间  inline namespace


using声明 及 using指示

using 声明，我们指示简单将令名字在局部作用域内有效

using 指示，令整个命名空间的所有内容变得有效
...
运行是类型识别
run-time tyep identification RTTI

只有能定义为虚函数，就定义虚函数

假设无法使用虚函数，可以使用一个RTTI运算符

使用RTTI要小心，最好是定义虚函数而不是直接接管  管理类型的问题

dynamic_cast 动态转换运算符
typeid       运算符
...
一个union 可以有多个数据成员，但是在任意时刻，只有一个数据成员可以有值

**extern "C"**

c++使用链接指示，指出任意非c++函数所使用的语言
```cpp
...
sizeof 定义一个空类型， sizeof的值为1

声明类型的实例的时候，必须在内存中有一定的空间，否则无法使用这些实例
空类型的实例占一个字节

添加构造函数和析构函数还是一样
调用构造和析构函数只需要函数的地址即可，函数的地址只与类型相关，而与类型的实例无关

虚函数，类型中有虚函数，就会为该类型生成虚函数表，并在该类型的每一个实例中添加一个指向虚函数表的指针。
32位  是 4
64位  是 8
...
对于内部定义的数据结构 char，int 而言两者的差距不大,效率相同

const成员必须在在初始化列表赋值

对于自定义的数据类型，函数初始化列表的效率要更好


#### 深拷贝vs浅拷贝

浅拷贝: 类中没有显示声明一个拷贝构造函数，编译器会自动生成一个拷贝构造函数，构造函数完成对象之间的位拷贝，这样的拷贝情况成为潜拷贝

深拷贝:如果一个类拥有资源，当这个类的对象发生复制过程的时候，资源需要重新分配

把对象所持有的其他资源一并拷贝的行为称位深拷贝

当类中有其他资源(动态分配内存，指向数据的指针)
这样必须使用深拷贝，只有这样才将指针内存再复制一份，原有对象和新生对象相互独立，不受影响

没有指针变量，一些简单的内置数据类型，使用浅拷贝
